      ...
      if($rezultat =@$polaczenie->query("SELECT * FROM artist WHERE name='$artist_name' and last_name='$artist_lname'") ){
        ...
        if($rezultat2 =@$polaczenie->query("SELECT * FROM `crew` INNER JOIN movies ON crew.title = movies.movie_title where name='$artist_name' and last_name='$artist_lname' order by release_date DESC ") ){
          if($rezultat2->num_rows>0){
            ...
            $datkaa[$i]= $wiersz2['release_date'];
            $dateek[$i] = DateTime::createFromFormat("Y-m-d", $datkaa[$i]);
            $ddk[$i] = $dateek[$i]->format("Y");
         ...
         $rolee .= "<li><div class ='aktor'><p class='para'><span class='span0'>$ddk[$xx]</span <span class='span1'><a class='axd' href = '../movies/$filmidd[$xx].php'>$movie_tit[$xx]</a></span><span class= 'span2'>$role[$xx]</span> </p></div></li>";
          }
